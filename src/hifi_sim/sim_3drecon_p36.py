...
class si3D(object):
    def __init__(self, image, nph, nangles, wavelength, na, dx=0.089, dz=0.2, **kwargs):
        ...
        if isinstance(image, (str, os.PathLike)):
            self.img_stack = tf.imread(
                image
            )  # order of images should be phases, angles, zslices
        ...
        )  # calculate pixel size in frequency space
        self.dpz = 1 / (
            (self.nz * 2.0) * (self.dz / 2.0)
        )  # calculate axial pixel size in frequency space
        self.radius_xy = (2 * self.na / self.wl) / self.dpx  # NA in pixels
        self.radius_z = ((self.na ** 2) / (2 * self.wl)) / self.dpz
        self.strength = (
            1.0  # determines strength of the zero-frequency suppression. 1 strongest?
        )
        self.sigma = 8.0  # variance. Used in freq suppression
    ...
    def separate(self, nang=0):
        ...
        self.img_1_0 = fftshift(
            self.interp((out[1] + 1j * out[2]).reshape(nz, nx, ny)) * self.winf
        )
        self.img_1_1 = fftshift(
            self.interp((out[1] - 1j * out[2]).reshape(nz, nx, ny)) * self.winf
        )
        self.img_2_0 = fftshift(
            self.interp((out[3] + 1j * out[4]).reshape(nz, nx, ny)) * self.winf
        )
        self.img_2_1 = fftshift(
            self.interp((out[3] - 1j * out[4]).reshape(nz, nx, ny)) * self.winf
        )
    ...
    def getoverlap1(self, angle, spacingx, spacingz, plot=False):
        ...
        ysh = np.exp(2j * np.pi * (kx * self.xv + ky * self.yv + 0.0 * self.zv))
        ...
        if plot == True:
            tf.imshow(
                np.abs((msk * wimgf1 * wimgf0.conj()) / (msk * wimgf0 * wimgf0.conj()))
            )
            tf.imshow(
                np.angle(
                    (msk * wimgf1 * wimgf0.conj()) / (msk * wimgf0 * wimgf0.conj())
                )
            )
    ...
    def getoverlapz(self, angle, spacingx, spacingz):
        ...
        ysh = self.shiftmat(0.0, kx, ky).astype(np.complex64)
        imgf = self.img_1_0.astype(np.complex64)
        imgf = fftn((imgf * ysh))
    ...
    def shift0(self, plot=False):
        ...
        zsp = self.zerosuppression(0.0, 0.0, 0.0)
        ...
        if plot == True:
            tf.imshow(
                np.abs(fftshift(self.otf0)),
                photometric="minisblack",
                title="Angle %d _ 0 order OTF" % self.nangle,
            )
            tf.imshow(
                np.abs(fftshift(self.imgf0)),
                photometric="minisblack",
    ...
    def pad(self, arr):
        ...
        if nz % 2 == 0:
            ...
            out[3 * nzh : 4 * nzh, 3 * nxh : 4 * nxh, 3 * nxh : 4 * nxh] = arr[
                nzh:nz, nxh:nx, nxh:nx
            ]
        else:
            nzh = np.int(nz / 2)
            out[:nzh, :nxh, :nxh] = arr[:nzh, :nxh, :nxh]
            ...
            out[(3 * nzh + 1) : (2 * nz), :nxh, 3 * nxh : 4 * nxh] = arr[
                nzh:nz, :nxh, nxh:nx
            ]
            out[(3 * nzh + 1) : (2 * nz), 3 * nxh : 4 * nxh, :nxh] = arr[
                nzh:nz, nxh:nx, :nxh
            ]
            out[(3 * nzh + 1) : (2 * nz), 3 * nxh : 4 * nxh, 3 * nxh : 4 * nxh] = arr[
                nzh:nz, nxh:nx, nxh:nx
            ]
        return out
    ...
    def zerosuppression(self, sz, sx, sy):
        ...
        g = 1 - self.strength * np.exp(
            -((x - sx) ** 2.0 + (y - sy) ** 2.0 + 0.0 * (z - sz) ** 2.0)
            / (2.0 * self.sigma ** 2.0)
        )  # See Karras2019 supplementary info S9 for the same equation
        g[g < 0.5] = 0.0
        g[g >= 0.5] = 1.0
        return g
    ...
    def apod(self):
        ...
        rxy = 2.0 * self.radius_xy
        rz = 2.0 * self.radius_z
        apo = (
            1 - self.axy * np.sqrt(self.xv ** 2 + self.yv ** 2) / rxy
        ) ** self.expn * (1 - self.az * np.sqrt(self.zv ** 2) / rz) ** self.expn
        rhxy = np.sqrt(self.xv ** 2 + self.yv ** 2 + 0.0 * self.zv ** 2) / rxy
        rhz = np.sqrt(0.0 * self.xv ** 2 + 0.0 * self.yv ** 2 + self.zv ** 2) / rz
    ...
    def recon1(self, phase1, mag1, phase2, mag2):
        ...
        ph1 = mag1 * np.exp(
            1j * phase1
        )  # complex phase gradient, represents the frequency shift?
        ...
        tf.imsave(
            "angle%d_imgf_0.tif" % self.nangle,
            np.abs(np.fft.fftshift(imgf)).astype(np.float32),
            photometric="minisblack",
        )
        otf = tf.imread(self.temp_join("otf_0.tif"))
        tf.imsave(
            "angle%d_otf_0.tif" % self.nangle,
            np.abs(np.fft.fftshift(otf)).astype(np.float32),
            photometric="minisblack",
        )
        ...
        tf.imsave(
            "angle%d_imgf_1_0.tif" % self.nangle,
            np.abs(np.fft.fftshift(imgf)).astype(np.float32),
            photometric="minisblack",
        )
        otf = tf.imread(self.temp_join("otf_1_0.tif"))
        tf.imsave(
            "angle%d_otf_1_0.tif" % self.nangle,
            np.abs(np.fft.fftshift(otf)).astype(np.float32),
            photometric="minisblack",
        )
        ...
        tf.imsave(
            "angle%d_imgf_1_1.tif" % self.nangle,
            np.abs(np.fft.fftshift(imgf)).astype(np.float32),
            photometric="minisblack",
        )
        otf = tf.imread(self.temp_join("otf_1_1.tif"))
        tf.imsave(
            "angle%d_otf_1_1.tif" % self.nangle,
            np.abs(np.fft.fftshift(otf)).astype(np.float32),
            photometric="minisblack",
        )
        ...
        tf.imsave(
            "angle%d_imgf_2_0.tif" % self.nangle,
            np.abs(np.fft.fftshift(imgf)).astype(np.float32),
            photometric="minisblack",
        )
        otf = tf.imread(self.temp_join("otf_2_0.tif"))
        tf.imsave(
            "angle%d_otf_2_0.tif" % self.nangle,
            np.abs(np.fft.fftshift(otf)).astype(np.float32),
            photometric="minisblack",
        )
        ...
        tf.imsave(
            "angle%d_imgf_2_1.tif" % self.nangle,
            np.abs(np.fft.fftshift(imgf)).astype(np.float32),
            photometric="minisblack",
        )
        otf = tf.imread(self.temp_join("otf_2_1.tif"))
        tf.imsave(
            "angle%d_otf_2_1.tif" % self.nangle,
            np.abs(np.fft.fftshift(otf)).astype(np.float32),
            photometric="minisblack",
        )
